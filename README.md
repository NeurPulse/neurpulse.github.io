...
### 1. 写文章（`npm run new` 使用教程）

不用手敲 frontmatter，一条命令自动生成：

**第 1 步**：在 VS Code 里打开本项目文件夹，按 `` Ctrl + ` ``（Tab 键上面的反引号）打开终端。
...
**第 3 步**：逐项回答（以写一篇 PyTorch 笔记为例，`→` 后是你输的内容）

```
类型  1=学习笔记  2=研究记录  [默认 1]:        → 直接回车（默认笔记）
标题:                                          → 深入浅出PyTorch：自动求导
文件名（英文小写+连字符）:                      → pytorch-autograd
分类:  1=数学建模  2=深度学习  3=深入浅出PyTorch  4=Transformer  5=论文精读  6=未分类
分类序号 [默认 未分类]:                          → 3
标签（逗号分隔，可留空）:                        → PyTorch, autograd
设为精选文章？(y/N):                            → 直接回车（默认不精选）
```
...
- 文件名想不出来可直接回车，自动生成（之后重命名文件即可改）
- 标签用中文或英文逗号分隔都行
- 提示"文件已存在"就换个文件名
- 写完不 push 也没关系，文件存在本地，push 了才会上线
...
- **文章目录**：根据正文里的 `##`、`###`、`####` 标题自动生成，显示在文章右侧（手机上看不到属正常，屏幕太窄时自动隐藏），滚动时会高亮当前小节
...
**Q: 想新增文章分类？**
改 `scripts/new-post.mjs` 里的 `CATEGORIES` 数组，加一项即可，立即生效。详见 [写作指南.md](写作指南.md) 第 7 节。
